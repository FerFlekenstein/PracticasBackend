...
class Contenedor {
    ...
    async save(objeto) {
        try {
            ...
            await fs.promises.writeFile(`./archivos/${this.nombre}`, infoJson)
            return objeto.id
        } catch (error) {
            console.log(error);
        }
    ...
    async getAll() {
        try {
            ...
            if (carpeta.includes(`${this.nombre}`)) {
                ...
                const hayInfo = await fs.promises.readFile(`./archivos/${this.nombre}`, 'utf-8')
                if (hayInfo) {
    ...
    async deleteAll() {
        try {
            await fs.promises.writeFile(`./archivos/${this.nombre}`, "")
        } catch (error) {
            console.log(error);
        }
    }
    async update(obj) {
        ...
        await fs.promises.writeFile(`./archivos/${this.nombre}`, todosActualizado)
    }
}
